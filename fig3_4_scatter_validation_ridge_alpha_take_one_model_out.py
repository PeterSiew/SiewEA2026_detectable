...
if __name__ == "__main__":
    ...
    alphas=[20000] # alpha=20000 yields the best result
    en_num=16
    ...
    for alpha in alphas:
        ## Load and save
        ## Y_test_trues are the same for all ensemlbes for a single model
    ...
    ### Calculate the 30-year trends from the targeted alpha=20000
    alpha=alphas[0]
    period=30
    model_vars=list(Y_test_trues_save[alpha].keys())
    ...
    ## Picking everying with alpha=0000
    Y_test_trues=np.array([Y_test_trues_save[alpha][var] for var in model_vars]).reshape(-1)
    Y_test_predicts=np.array([Y_test_predicts_save[alpha][var] for var in model_vars]).reshape(-1)
    ...
    ### Start plotting (Figure 3; the timeseries validatoin)
    ...
    for i, var in enumerate(model_vars):
        ...
        axs[i].plot(x,Y_org_slopes[var][en],color='k',label='Raw',lw=0.5,linestyle='-')
        ...
        rmse_label="RMSE=%s, %s=%s"%(str(round(rmse,3)),"r",str(round(corr,2)))
    ...
    corrs_save=[]
    for var in model_vars:
        #if var!=model_vars[3]: # Only plot the first var
        #    continue
        ## Plot the scatter
        true_plot=np.array(trues[var]).flatten() # it has a size of 1x16x121 (model*members*no of slicing period)
        ...
        ## Calculate correlations and RMSE for indiviudal models
        ...
        corrs_save.append(corr_predict_true)
        #rmse_label=" (RMSE: %s %s %s)"%(str(round(rmse_org_true,3)),r"$ \Longrightarrow$",str(round(rmse_predict_true,3)))
        rmse_label=" (RMSE=%s, %s=%s)"%(str(round(rmse_predict_true,3)),"r",str(round(corr_predict_true,2)))
        ax2.scatter([-100],[-100], s=4,color=colors[var],label=vars_labels[var]+rmse_label)
        if False: ## Add the best-fit line for individual models
            slope, intercept, r, p, se = scipy.stats.linregress(true_plot, predict_plot)
            y_fit=slope*true_plot+intercept
            ax2.plot(true_plot,y_fit,color=colors[var],lw=3)
        ## Save them for calculating RMSE and corr for all simulations
    ...
    slope, intercept, r, p, se = scipy.stats.linregress(true_true,predict_predict)
    print('Slope: ',slope, 'intercept:',intercept)
    #true_true=np.append(true_true,-10); true_true=np.append(true_true,10)
    y_fit=slope*true_true+intercept
    ax2.plot(true_true,y_fit,color='k',lw=3)
    ## Get the absolute difference for all predictions and true values 
